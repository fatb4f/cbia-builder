# Implementation Progression DAG — Pipeline-SRE Skill (Regenerated)

This DAG models the **implementation progression** for the Pipeline-SRE skill, aligned to milestones **M0–M5**, plus deferred refinement branches.
...
**G0 pass**
- All files exist, parse, and include required keys (no placeholders).

### M1 — Mechanical constraint implementations
**Required artifacts**
- `tools/structure_ops_preflight.py` (or equivalent) implements constraint IDs in catalog
- emits stable evidence JSON with reason codes

**G1 pass**
- Every constraint ID resolves to an executable function.
- Evidence schema is stable (keys present, typed).

### M2 — Orchestrator
**Required artifacts**
- `tools/orchestrator.py` loads skill SSOT and runs `dag.json`
- **coverage gate** denies missing required primitives

**G2 pass**
- Deterministic for fixed input bundle.
- Fail-closed merge: any error/missing observable ⇒ DENY.
- Emits: `artifacts/pipeline-sre/{decision.json,explain.json,evidence/**}`.

### M3 — CI wiring
**Required artifacts**
- workflow step invoking orchestrator as the single entrypoint
- evidence artifacts uploaded

**G3 pass**
- checkout uses `fetch-depth: 0`.
- orchestrator runs on PRs and emits evidence even on deny.
...
**Required artifacts**
- generated “End-state constraints (SSOT)” block in `SKILL.md`
- `end_state.json` hash is recorded + verified during runs

**G5 pass**
- Any drift between instruction block and SSOT hash ⇒ DENY.
...
## Notes
- This is a **plan DAG** (implementation progression), not the runtime Pipeline-SRE execution DAG.
- It is compatible with a future mechanical checklist runner that asserts artifact presence and CI wiring matches the plan.
